 ...
 * 对应前端页面中的术后检查
 */
public class MakeAfterMedicalTreatment {
    public static String make(ReceivePathway receivePathway){
        String afterDuration = receivePathway.getAfterDuration();
        String afterDuration_treatment_min = receivePathway.getAfterDuration_treatment_min();
        String afterDuration_treatment_max = receivePathway.getAfterDuration_treatment_max();
        String afterDuration_treatment_text = receivePathway.getAfterDuration_treatment_text();
        String after_medicalScenario = receivePathway.getAfter_medicalScenario();
        //定义总对象
...
//            } else {
                ...
                for (int j = 0; j < obligatory_exam_num; j++) {
                    //首先判断该字段不为""
                    if (!"".equals(obligatory_exam_s.getJSONObject(j).getString("value"))) {
                ...
                for (int j = 0; j < optional_exam_num; j++) {
                    //首先判断该字段不为""
                    if (!"".equals(optional_exam_s.getJSONObject(j).getString("value"))) {
                ...
                for (int j = 0; j < recovery_plan_num; j++) {
                    //首先判断该字段不为""
                    if (!"".equals(recovery_plan_s.getJSONObject(j).getString("value"))) {
        ...
        return After_medical_treatment.toString();
    }
}
